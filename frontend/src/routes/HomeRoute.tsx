...
import { Badge, Button, Card, Dialog, Select, Skeleton } from "../components/ui/primitives";
...
function BillRow({ row }: { row: BillListRow }) {
  const state = status(row);
  return <div className="history-row bill-row" role="button" tabIndex={0} data-id={row.id} aria-label={`Buka bill ${row.title}, ${state.label}`} onClick={() => navigate({ kind: "bill", billId: row.id })} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); navigate({ kind: "bill", billId: row.id }); } }}>
    <div className={`avatar status-mark status-${state.tone}`} aria-hidden="true"><Receipt /></div>
    <div className="bill-row-main"><div className="bill-row-title"><strong>{row.title}</strong><span className="caption bill-row-date">{shortDate(billDate(row))}</span></div><div className="bill-row-meta"><Badge className="chip" tone={state.tone}>{state.label}</Badge><span className="money bill-row-amount">{rupiahFmt(row.total_idr)}</span></div>{!isSettled(row) && row.i_am_payer && <div className="item-share">Kamu yang nalangin</div>}{!isSettled(row) && !row.i_am_payer && row.has_picks && !row.my_paid && <div className="item-share">Kamu belum bayar</div>}</div>
    {row.can_manage && <Button type="button" variant="ghost" size="icon" className="delete-bill" aria-label={`Hapus bill ${row.title}`} onClick={event => { event.stopPropagation(); }}><Trash /></Button>}
  </div>;
}
