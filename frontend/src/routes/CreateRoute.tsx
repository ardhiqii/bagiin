...
function CreateStart({ onManual, onPhoto }: { onManual: () => void; onPhoto: () => void }) {
  return <AppFrame contextualDock><Topbar title="Buat bill" back={() => navigate({ kind: "home" })} /><main className="shell"><div className="page-intro"><p className="eyebrow">Mulai patungan</p><h1>Pilih cara masukin struk.</h1><p className="muted">Foto struk untuk dibaca otomatis, atau isi item satu per satu.</p></div><div className="choice-grid"><Card id="dz"><ShadcnButton id="ocr-btn" className="btn-block" onClick={onPhoto}><Camera /> Foto struk</ShadcnButton><p className="muted">Bisa menambahkan sampai dua foto.</p></Card><Card><ShadcnButton id="manual-btn" variant="outline" className="btn-block" onClick={onManual}><PencilSimple /> Isi manual</ShadcnButton><p className="muted">Kamu tetap bisa menambahkan foto nanti.</p></Card></div></main></AppFrame>;
}
...
function VerifyEditor({ draft, updateDraft, cleanup, onBack, onSubmit, busy, error, onError }: { draft: BillDraft; updateDraft: (updater: (current: BillDraft) => BillDraft) => void; cleanup: PhotoCleanup; onBack: () => void; onSubmit: (event: React.FormEvent) => void; busy: boolean; error: string; onError: (error: string) => void }) {
  ...
  </div><aside className="shell-side"><div className="dock"><div className="dock-panel dock-panel-create"><div className="dock-total"><span className="muted">Total baris dihitung</span><strong id="total-display" className="money">{total == null ? "-" : rupiahFmt(total)}</strong></div><div id="sum-warn" className="caption">{subtotal == null ? "Lengkapi jumlah item untuk melihat total." : ""}</div><Button id="create-bill-btn" type="submit" className="btn-block" disabled={!valid || busy} onClick={onSubmit}>{busy ? <><Spinner /> Membuat...</> : <><Check /> Buat bill</>}</Button></div></div></aside></main></AppFrame>;
}
