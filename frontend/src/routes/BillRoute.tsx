...
function EditItemRow({ item, canDelete, onChange, onDelete }: { item: EditItemDraft; canDelete: boolean; onChange: (patch: Partial<EditItemDraft>) => void; onDelete: () => void }) {
  ...
  return <div className="vf-item" data-edit-item={item.id ?? item.key}>
    ...
    <div className="vf-input-group form-grid"><div className="field"><Label htmlFor={itemPriceId}>Harga satuan</Label><Input id={itemPriceId} data-role="price" inputMode="numeric" value={editMoneyInput(item.price)} onChange={event => onChange({ price: editMoney(event.currentTarget.value) })} placeholder="0" /></div><div className="field"><Label htmlFor={itemQuantityId}>Jumlah dibeli</Label><div className="qty-control"><Button type="button" className="edit-qty-dec" variant="outline" size="icon" style={{ width: 44, minWidth: 44 }} aria-label="Kurangi jumlah dibeli" disabled={currentQuantity === 1} onClick={() => onChange({ quantity: Math.max(1, (currentQuantity || 1) - 1), quantityDraft: null })}>−</Button><Input id={itemQuantityId} data-role="quantity" type="number" inputMode="numeric" min={1} max={99} step={1} value={quantityText} aria-invalid={quantityInvalid} aria-describedby={`${itemQuantityId}-error`} onChange={event => { const raw = event.currentTarget.value; const parsed = editQuantity(raw); onChange(parsed == null ? { quantityDraft: raw } : { quantity: parsed, quantityDraft: null }); }} /><Button type="button" className="edit-qty-inc" variant="outline" size="icon" style={{ width: 44, minWidth: 44 }} aria-label="Tambah jumlah dibeli" disabled={currentQuantity === 99} onClick={() => onChange({ quantity: Math.min(99, (currentQuantity || 0) + 1), quantityDraft: null })}>+</Button></div><p id={`${itemQuantityId}-error`} data-role="quantity-error" className={`error-text${quantityInvalid ? "" : " hidden"}`} aria-live="polite" hidden={!quantityInvalid}>Jumlah harus bilangan bulat 1–99.</p></div></div>
