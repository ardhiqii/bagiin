    ...
    <div class="card" style="margin-bottom:12px;background:var(--surface-2,#141414);">
      <div style="display:flex;justify-content:space-between;align-items:center;">
        <div>
          ...
          <div class="money" style="font-size:22px;font-weight:800;">${fmt(computeMyTotal(data, mySel, me.id))}</div>
        </div>
      ...
      <div style="display:flex;justify-content:space-between;margin-top:8px;font-size:13px;color:var(--text-2,#bbb);">
        <span>Item <span class="money-sm">${fmt(computeMyBreakdown(data, mySel).sub)}</span></span>
        <span>Pajak & service <span class="money-sm" style="color:var(--accent,#f97316);">${fmt(computeMyBreakdown(data, mySel).tax)}</span></span>
      </div>` : ""}
    </div>` : ""}
    <div class="card">
...
function renderCreatorPick(data) {
  ...
  appEl.innerHTML = `
    ...
    <div class="card" style="margin-bottom:12px;">
      ...
      ${data.bill.merchant ? `<p class="muted" style="margin-top:6px;">${esc(data.bill.merchant)}</p>` : ""}
      <p class="muted" style="margin-top:6px;">Ketuk item yang kamu tanggung. Item slot bisa diambil lebih dari 1 slot. Yang gak dicentang siapa-siapa masuk ke kamu.</p>
    </div>
    <div class="card">
      <div class="card-title">Centang yang kamu tanggung</div>
      <div id="pick-items">${data.items.map(it => itemRowHtml(it, data, mySel, me.name, me)).join("")}</div>
...
function openCloseConfirm(data) {
  const notPicked = data.people.filter(p => !p.subtotal_idr && p.identity_id !== data.owner_id);
