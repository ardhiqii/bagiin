...
// status chip: Selesai (closed) / Lunas (open, all paid) / Belum lunas (open)
...
function photoBtnHtml(data, me) {
  const isOwner = data.owner_id === me.id;
  ...
  if (isOwner && data.bill.status === "open") {
...
function renderGuestView(data, me) {
  ...
  const bdGuest = computeMyBreakdown(data, mySel);
  ...
  appEl.innerHTML = `
    ...
    ${data.bill.status === "closed" ? `
    <div class="card" style="margin-bottom:12px;background:var(--surface-2,#141414);">
      <div style="display:flex;justify-content:space-between;align-items:center;">
        <div>
          <div class="label-sm">Total kamu</div>
          <div class="money" style="font-size:22px;font-weight:800;">${fmt(myPeople.length ? myPeople[0].total_idr : computeMyTotal(data, mySel, me.id))}</div>
        </div>
        <span class="chip ${myPaid ? "chip-green" : "chip-red"}">${myPaid ? "✓ udah bayar" : "belum bayar"}</span>
      </div>
      ${taxServiceTotal(data) > 0 ? `
      <div style="display:flex;justify-content:space-between;margin-top:8px;font-size:13px;color:var(--text-2,#bbb);">
        <span>Item <span class="money-sm">${fmt(myPeople.length ? myPeople[0].subtotal_idr : computeMyBreakdown(data, mySel).sub)}</span></span>
        <span>Pajak & service <span class="money-sm" style="color:var(--accent,#f97316);">${fmt(myPeople.length ? myPeople[0].tax_idr : computeMyBreakdown(data, mySel).tax)}</span></span>
      </div>` : ""}
    </div>` : ""}
    <div class="card">
      <div class="card-title">Centang yang kamu tanggung</div>
      <div id="pick-items">${data.items.map(it => itemRowHtml(it, data, mySel, me.name, me, data.bill.status === "closed")).join("")}</div>
    </div>
    ...
    <div class="sticky-bar"><div class="sticky-inner" style="flex-direction:column;align-items:stretch;gap:10px;">
      ...
      <div id="my-breakdown" style="display:${taxServiceTotal(data) > 0 ? "flex" : "none"};justify-content:space-between;align-items:center;flex:none;background:var(--surface-2,#141414);border:1px solid var(--border,#2a2a2a);border-radius:10px;padding:8px 12px;font-size:13px;gap:8px;">
        <span style="color:var(--text-2,#bbb);display:flex;align-items:center;gap:6px;min-width:0;">
          <span style="flex-shrink:0;">Item</span>
          <span id="my-sub" class="money-sm">${fmt(bdGuest.sub)}</span>
        </span>
        <span style="color:var(--text-2,#bbb);display:flex;align-items:center;gap:6px;min-width:0;text-align:right;">
          <span style="flex-shrink:0;">Pajak &amp; service</span>
          <span id="my-tax" class="money-sm" style="color:var(--accent,#f97316);">${fmt(bdGuest.tax)}</span>
        </span>
      </div>
      ${iAmPayer
...
function itemRowHtml(it, data, mySel, myName, me, readOnly) {
  ...
  const priceHtml = it.discount_idr > 0
    ...
    : fmt(eff);
  return `
    <div class="item-row ${isSel ? "selected" : ""}${readOnly ? " item-row-ro" : ""}" data-item="${it.id}"${readOnly ? ` style="opacity:.8;cursor:default;"` : ""}>
...
function bindItemRows(data, me, root) {
  ...
  $$("#pick-items .item-row", scope).forEach(row => {
    if (state.bill.status === "closed") return; // read-only rows, no affordance
...
function renderPickRows(data, me) {
  ...
  const mb = $("div#my-breakdown, #my-breakdown");
  const mbEl = mb && mb.length ? mb[0] : mb;
  if (mbEl) {
    const hasTax = taxServiceTotal(data) > 0;
    mbEl.style.display = hasTax ? "flex" : "none";
    const subEl = $("span#my-sub", mbEl);
    const taxEl = $("span#my-tax", mbEl);
  ...
  const sheet = el(`
    <div class="sheet-overlay" id="free-picker-sheet">
      <div class="sheet">
        ...
        <p class="muted" style="margin-bottom:14px;">${statusText}</p>
        <div style="display:flex;align-items:center;justify-content:center;gap:20px;margin-bottom:16px;">
      ...
      <div class="card" style="background:var(--surface-2);border:none;margin-bottom:12px;">
        ...
        ${items.map(it => {
          ...
          if (it.mode === "slot" && it.slot_count) {
            ...
            const othersN = othersSel(data.sel_by_item[it.id], me).reduce((s, x) => s + (x.qty || 1), 0);
...
function renderCreatorView(data) {
  ...
  if (data.warnings.length) {
    ...
    // Skip "Bagian kosong: ..." — already shown as its own 🧩 row above.
    ...
    const dedup = data.warnings
      ...
      .replace(/(\d),(\d{3})/g, "$1.$2")
      .replace(/->/g, "→");
    if (dedup) warnRows.push({ icon: "🧾", text: esc(dedup) });
  }
  if (notPicked.length) {
  ...
  app.innerHTML = `
    ...
    <div class="card" style="margin-bottom:12px;">
      ...
      <div style="display:flex;gap:16px;margin-top:12px;">
        ...
        <div>${data.settled
          ? `<span class="chip chip-green">semua lunas</span>`
          : (totalUnpaid > 0
            ? `<span class="chip chip-red">${fmt(totalUnpaid)} belum</span>`
            : (data.uncovered_idr > 0
              ? `<span class="chip chip-grey">bagian kosong tersisa</span>`
              : `<span class="chip chip-grey">Belum ada yang milih</span>`))}</div>
      </div>
      ${photoBtnHtml(data, me)}
        ...
        ${data.people.map(p => `
          <div class="person-row">
            ...
            <div class="person-info">
              ...
              <div class="person-sub">${(p.subtotal_idr || hasPickedAny(data, p.identity_id))
                ? (p.subtotal_idr
                  ? `${fmt(p.subtotal_idr)} item · ${fmt(p.tax_idr)} pajak`
                  : `item gratis · ${fmt(p.tax_idr)} pajak`)
                : "belum pilih item"}</div>
            </div>
...
function openSlotManagerSheet(data, it) {
  ...
  $$(".mgr-free", sheet).forEach(b => b.addEventListener("click", async () => {
    // native confirm() is plain text — esc() would show "&amp;" entities
    if (!confirm(`Lepas slot ${b.dataset.name} dari "${it.name}"?`)) return;
    try {
      ...
      <div class="sheet">
        ...
        <div style="display:flex;flex-direction:column;gap:8px;">
          <button class="btn-outline payer-opt" data-id="${esc(data.bill.creator_identity_id)}" data-name="${esc(data.creator_name)}" style="text-align:left;justify-content:flex-start;">
            ${data.paid_by_id === data.bill.creator_identity_id ? "✓ " : ""}<strong>${esc(data.creator_name)}</strong> <span class="muted">(${data.owner_id === data.bill.creator_identity_id ? "kamu, pembuat" : "pembuat"})</span>
          </button>
...
function renderCreatorPick(data) {
  ...
  appEl.innerHTML = `
    ...
    <div class="card" style="margin-bottom:12px;">
      ...
      ${data.bill.merchant ? `<p class="muted" style="margin-top:6px;">${esc(data.bill.merchant)}</p>` : ""}
      <p class="muted" style="margin-top:6px;">Ketuk item yang kamu tanggung. Item slot bisa diambil lebih dari 1 slot. Item bebas yang gak dicentang siapa-siapa masuk ke kamu; bagian slot yang kosong tetep keliatan buat diambil.</p>
    </div>
    <div class="card">
      <div class="card-title">Centang yang kamu tanggung</div>
    ...
    <div class="sticky-bar"><div class="sticky-inner" style="flex-direction:column;align-items:stretch;gap:10px;">
      ...
      <div id="my-breakdown" style="display:${taxServiceTotal(data) > 0 ? "flex" : "none"};justify-content:space-between;align-items:center;flex:none;background:var(--surface-2,#141414);border:1px solid var(--border,#2a2a2a);border-radius:10px;padding:8px 12px;font-size:13px;gap:8px;">
        <span style="color:var(--text-2,#bbb);display:flex;align-items:center;gap:6px;min-width:0;">
          <span style="flex-shrink:0;">Item</span>
          <span id="my-sub" class="money-sm">${fmt(computeMyBreakdown(data, mySel).sub)}</span>
        </span>
        <span style="color:var(--text-2,#bbb);display:flex;align-items:center;gap:6px;min-width:0;text-align:right;">
          <span style="flex-shrink:0;">Pajak &amp; service</span>
          <span id="my-tax" class="money-sm" style="color:var(--accent,#f97316);">${fmt(computeMyBreakdown(data, mySel).tax)}</span>
        </span>
      </div>
...
function openCloseConfirm(data) {
  const notPicked = data.people.filter(p => !p.subtotal_idr && !hasPickedAny(data, p.identity_id) && p.identity_id !== data.owner_id);
