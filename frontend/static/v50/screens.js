  ...
  app.innerHTML = `
    <div style="min-height:70dvh;display:flex;flex-direction:column;justify-content:center;max-width:400px;margin:0 auto;">
      ...
      <div class="field">
        ...
        <input id="name-input" type="text" placeholder="Biar temenmu tau ini kamu" value="${esc(saved)}" maxlength="30" autocomplete="off">
      </div>
...
function renderHome() {
  ...
  const name = esc(state.identity.name.split(" ")[0]);
  app.innerHTML = `
    <div class="topbar">
...
function personalStatusHtml(b) {
  ...
  return b.my_paid
    ? `<div class="muted" style="color:var(--green);font-size:12px;margin-top:2px;">✓ Kamu udah bayar</div>`
    : `<div class="muted" style="color:var(--red);font-size:12px;margin-top:2px;">Kamu belum bayar</div>`;
}
  ...
  try {
    ...
    box.innerHTML = bills.slice(0, 5).map(b => `
      <div class="history-row" data-id="${b.id}">
        ...
        ${b.owner_id === state.identity.id ? `<button class="btn-sm delete-bill" data-id="${b.id}" data-title="${esc(b.title)}" title="Hapus bill" style="background:var(--red-bg);color:var(--red);flex-shrink:0;">🗑️</button>` : ""}
      </div>`).join("");
...
function renderHistory() {
  ...
  (async () => {
    try {
      ...
      const box = $("#hist-list");
      if (box) box.innerHTML = `<div class="muted" style="text-align:center;padding:12px;">Gagal muat: ${esc(e.message)}</div>`;
      toast(e.message);
    }
  })();
}
...
function renderCreate(opts = {}) {
  ...
  app.innerHTML = `
    ...
    <div id="create-body">
      ...
      <div class="dropzone" id="dz">
        ...
        <div class="muted">Ketuk, tempel (Ctrl+V), atau tarik file ke sini</div>
      </div>
      <input type="file" id="file-input" accept="image/*" class="hidden">
      <button class="btn-outline" id="manual-btn" style="width:100%;margin-top:12px;">✍️ Bikin Manual (tanpa foto)</button>
    </div>`;
  $("button#back-btn").addEventListener("click", () => location.hash = "#/");
  ...
  dz.addEventListener("click", () => {
    // bottom sheet: [Ambil Foto] [Pilih dari Galeri] [Tempel dari Clipboard]
    const sheet = el(`
      <div class="sheet-overlay" id="dz-sheet">
        <div class="sheet">
          ...
          <button class="btn-outline" id="dz-clipboard" style="width:100%;margin-bottom:8px;">📋 Tempel dari Clipboard</button>
          <button class="btn-ghost" id="dz-cancel" style="width:100%;">Batal</button>
        </div>
      </div>`);
    document.body.appendChild(sheet);
    $("button#dz-camera", sheet).addEventListener("click", () => { sheet.remove(); openPicker(true); });
    $("button#dz-gallery", sheet).addEventListener("click", () => { sheet.remove(); openPicker(false); });
    $("button#dz-clipboard", sheet).addEventListener("click", () => { sheet.remove(); readClipboardImage(); });
    $("button#dz-cancel", sheet).addEventListener("click", () => sheet.remove());
    sheet.addEventListener("click", (e) => { if (e.target === sheet) sheet.remove(); });
  });
...
async function readClipboardImage() {
  try {
    if (!navigator.clipboard || !navigator.clipboard.read) {
      toast("Browser gak support baca clipboard — coba long-press terus pilih Paste");
      return;
    }
    ...
    toast("Clipboard-nya gak ada gambar");
  } catch (e) {
...
function renderVerify(ocr, manual = false) {
  ...
  app.innerHTML = `
    ...
    <div class="card">
      ...
      <div id="paid-by-other" class="${verifyState.paid_by_name ? "" : "hidden"}" style="margin-top:10px;">
        ...
        <input id="paid-by-name-input" placeholder="Contoh: Budi" value="${esc(verifyState.paid_by_name || "")}" maxlength="30" autocomplete="off" style="margin-top:4px;">
  ...
  const paidByMe = $("div#paid-by-me, #paid-by-me");
  if (paidByMe) {
    paidByMe.addEventListener("change", () => {
      $("div#paid-by-other, #paid-by-other").classList.toggle("hidden", paidByMe.checked);
      if (paidByMe.checked) {
        // re-checking "saya yang bayar" must clear a stale typed name,
  ...
  const paidByNameInput = $("div#paid-by-name-input, #paid-by-name-input");
  if (paidByNameInput) {
    paidByNameInput.addEventListener("input", (e) => {
      if (paidByMe && paidByMe.checked) return; // ignore while "saya" checked
