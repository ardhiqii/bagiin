...
function renderGuestView(data, me) {
  ...
  Object.entries(data.sel_by_item || {}).forEach(([itemId, selList]) => {
    const mine = selList.find(s => normName(s.name) === normName(me.name));
  ...
  appEl.innerHTML = `
    ...
    <div class="card" style="margin-bottom:12px;">
      ...
      ${data.uncovered_idr > 0 ? `<p class="muted" style="margin-top:6px;color:var(--red);">⚠️ Ada ${data.uncovered_slots.length ? data.uncovered_slots.reduce((s, u) => s + u.empty, 0) : "?"} slot kosong belum keambil (${fmt(data.uncovered_idr)})</p>` : ""}
    ...
    <div class="card">
      ...
      <div id="pick-items">${data.items.map(it => itemRowHtml(it, data, mySel, me.name)).join("")}</div>
    </div>
...
function itemRowHtml(it, data, mySel, myName) {
  const selList = (data.sel_by_item[it.id] || []);
  const mine = selList.find(s => normName(s.name) === normName(myName));
  const myQty = mine ? (mine.qty || 1) : 0;
  const isSel = myQty > 0;
  const isSlot = it.mode === "slot" && it.slot_count;
  let shareText;
  if (isSlot) {
    const perSlot = Math.floor(it.price_idr / it.slot_count);
    const taken = selList.reduce((s, x) => s + (x.qty || 1), 0);
    const empty = Math.max(0, it.slot_count - taken);
    const mineTxt = myQty > 0 ? ` · kamu ${myQty}×` : "";
    shareText = `${taken}/${it.slot_count} slot · ${fmt(perSlot)}/slot${mineTxt}${empty > 0 ? ` · ${empty} kosong` : ""}`;
  } else if (selList.length === 0) {
    shareText = "belum dipilih";
  } else if (selList.length === 1) {
    shareText = mine ? "kamu" : selList[0].name;
  } else if (mine) {
    shareText = `dishare ${selList.length} orang (termasuk kamu): ${selList.map(s => esc(s.name)).join(", ")}`;
  } else {
    shareText = `dishare ${selList.length} orang: ${selList.map(s => esc(s.name)).join(", ")}`;
  }
  return `
...
async function updateGuestSelection(data, me) {
  ...
  $$("#pick-items .item-row").forEach(row => {
    ...
    if (share) {
      const it = data.items.find(x => x.id === id);
      const selList = (data.sel_by_item[id] || []).filter(s => normName(s.name) !== normName(me.name));
      if (it && it.mode === "slot" && it.slot_count) {
        const taken = selList.reduce((s, x) => s + (x.qty || 1), 0) + qty;
        const empty = Math.max(0, it.slot_count - taken);
        share.textContent = `${taken}/${it.slot_count} slot${qty > 0 ? ` · kamu ${qty}×` : ""}${empty > 0 ? ` · ${empty} kosong` : ""}`;
      } else if (sel) {
        share.textContent = selList.length > 0 ? `dishare ${selList.length + 1} orang (termasuk kamu)` : "kamu";
      } else {
        share.textContent = selList.length > 0 ? selList.map(s => s.name).join(", ") : "belum dipilih";
      }
    }
  });
  ...
  selectionSaveChain = selectionSaveChain
    .then(() => apiJson(`/api/bills/${data.bill.id}/selections`, "POST", { picks }))
    .catch(e => toast(e.message));
...
function openSlotPickerSheet(data, me, it) {
  const selList = (data.sel_by_item[it.id] || []);
  const othersTaken = selList
    .filter(s => normName(s.name) !== normName(me.name))
    .reduce((s, x) => s + (x.qty || 1), 0);
  const myQty = (selList.find(s => normName(s.name) === normName(me.name)) || {}).qty || 0;
  ...
  const sheet = el(`
    <div class="sheet-overlay" id="slot-picker-sheet">
      <div class="sheet">
        ...
        <p class="muted" style="margin-bottom:4px;">${fmt(perSlot)}/slot · ${it.slot_count} slot · ${othersTaken} slot keambil orang lain</p>
        <p class="muted" style="margin-bottom:14px;">${leftEmpty > 0 ? `Sisa slot kosong: ${leftEmpty}` : "Slot udah abis"}</p>
        ${max > 0 ? `
  ...
  if (max > 0) {
    ...
    $("button#slot-confirm", sheet).addEventListener("click", async () => {
      try {
        await apiJson(`/api/bills/${data.bill.id}/selections`, "POST", {
          picks: [...state.selQty.entries()].filter(([iid]) => iid !== it.id).map(([iid, q]) => ({ item_id: iid, qty: q }))
...
function computeMyBreakdown(data, selQty) {
  ...
  data.items.forEach(it => {
    ...
    const selList = (data.sel_by_item[it.id] || []);
    if (!myQty) {
      if (selList.length > 0) totalSelAll += it.price_idr;
      return;
    }
    // free item: count = existing selectors that aren't me (by id or name), +1 me
    let count = selList.filter(s => (s.id && s.id !== myId) ? true : normName(s.name) !== normName(myName)).length;
    count += 1;
    if (count <= 0) return;
    sub += Math.floor(it.price_idr / count);
    totalSelAll += it.price_idr;
  });
      ...
      <div class="card" style="background:var(--surface-2);border:none;margin-bottom:12px;">
        ...
        ${items.map(it => {
          ...
          if (it.mode === "slot" && it.slot_count) {
            ...
            myPrice = n > 1 ? Math.floor(it.price_idr / n) : it.price_idr;
            shareNote = n > 1 ? `dibagi ${n}` : "";
          }
          return `
          <div class="pay-item" data-item="${it.id}">
            ...
            <div style="text-align:right;flex-shrink:0;">
              ...
              ${it.mode !== "slot" && (data.sel_by_item[it.id] || []).length > 1 ? `<div class="muted" style="font-size:11px;">dari ${fmt(it.price_idr)}</div>` : ""}
            </div>
...
function renderCreatorView(data) {
  ...
  if (data.uncovered_slots.length) {
    warnRows.push({
      icon: "🧩",
      text: `Slot kosong belum keambil: ${data.uncovered_slots.map(u => `${esc(u.name)} (${u.empty} × ${fmt(u.per_slot)} = ${fmt(u.amount_idr)})`).join(", ")}`,
      red: true,
    });
  }
    ...
    <div class="card">
      ...
      ${data.items.map(it => {
        ...
        if (isSlot) {
          ...
          const taken = selList.reduce((s, x) => s + (x.qty || 1), 0);
          const empty = Math.max(0, it.slot_count - taken);
          shareText = `${taken}/${it.slot_count} slot · ${fmt(perSlot)}/slot`
        ...
        <div class="item-row" style="border-bottom:1px solid var(--border);">
          ...
          <div class="money item-price">${fmt(it.price_idr)}</div>
          ${data.bill.status === "open" && isSlot ? `<button class="btn-sm slot-mgr" data-item="${it.id}" data-name="${esc(it.name)}" title="Atur slot" style="flex-shrink:0;margin-left:6px;">⚙️</button>` : ""}
        </div>`;
      }).join("")}
    </div>
    ${data.bill.status === "open" ? `
...
function openSlotManagerSheet(data, it) {
  ...
  const sheet = el(`
    <div class="sheet-overlay" id="slot-mgr-sheet">
      <div class="sheet">
        <div class="sheet-handle"></div>
        <div class="sheet-title">Atur slot: ${esc(it.name)}</div>
        <p class="muted" style="margin-bottom:12px;">${fmt(it.price_idr)} · ${fmt(perSlot)}/slot · ${taken}/${n} keambil</p>
        ...
        <p class="muted" style="font-size:13px;margin-bottom:12px;">Slot minimal ${taken} (yang udah keambil). Harga per slot ngikut total slot.</p>
        <button class="btn-primary" id="mgr-save" style="width:100%;">Simpan</button>
        ${selList.length ? `<div class="label-sm" style="margin:14px 0 6px;">Pemegang slot</div>` : ""}
        ${selList.map(s => `
          <div class="account-row">
...
function renderCreatorPick(data) {
  ...
  const me = state.identity;
  state.bill = data;
  // build my picks (item_id -> qty) for creator (match by name like guest view)
  const mySel = new Map();
  Object.entries(data.sel_by_item || {}).forEach(([itemId, selList]) => {
    const mine = selList.find(s => normName(s.name) === normName(me.name));
    if (mine) mySel.set(parseInt(itemId, 10), mine.qty || 1);
...
function renderEditBill(data) {
  const app = $("#app");
  editState = {
    items: data.items.map(it => ({ id: it.id, name: it.name, price: it.price_idr, mode: it.mode || "free", slot_count: it.slot_count || null })),
...
function renderEditItems() {
  ...
  elList.innerHTML = editState.items.map((it, idx) => `
    <div class="item-row" style="border-bottom:1px solid var(--border);flex-wrap:wrap;">
      ...
      <div style="flex-basis:100%;display:flex;align-items:center;gap:8px;padding:6px 0 8px;">
        <span class="label-sm" style="flex-shrink:0;">Mode:</span>
...
function openCloseConfirm(data) {
  ...
  const singles = (data.sel_by_item ? Object.entries(data.sel_by_item).filter(([id, selList]) => selList.length === 1 && selList[0].qty === 1) : []);
  ...
  const sheet = el(`
    <div class="sheet-overlay" id="close-sheet">
      <div class="sheet">
        ...
        ${emptySlots.length ? `<div class="warn-box" style="color:var(--red);"><strong>Slot kosong belum keambil:</strong><ul>
          ${emptySlots.map(u => `<li>${esc(u.name)} — ${u.empty} slot (${fmt(u.amount_idr)})</li>`).join("")}
        </ul></div>` : ""}
