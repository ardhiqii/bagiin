  ...
  const sheet = el(`
    <div class="sheet-overlay" id="slot-mgr-sheet">
      <div class="sheet">
        ...
        <p class="muted" style="margin-bottom:12px;">${fmt(it.price_idr)} · ${fmt(perSlot)}/bagian · ${taken}/${n} keambil</p>
...
function renderEditItems() {
  ...
  elList.innerHTML = editState.items.map((it, idx) => `
    <div class="item-row" style="border-bottom:1px solid var(--border);flex-wrap:wrap;">
      ...
      <div style="flex-basis:100%;padding:6px 0 8px;">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
          <span class="label-sm" style="flex-shrink:0;">Cara bagi:</span>
          <button class="btn-sm item-mode-btn ${it.mode !== "slot" ? "chip-active" : ""}" data-idx="${idx}" data-mode="free" style="background:${it.mode !== "slot" ? "var(--surface-2)" : "var(--bg)"};border:1px solid var(--border);">Bagi rata</button>
        ...
        <div class="muted" style="font-size:12px;line-height:1.45;">
          ${it.mode === "slot"
            ...
            : `Semua yang centang item ini kebagian rata. Cocok buat yang dimakan bareng.`}
        </div>
      </div>
    </div>`).join("");
