  ...
  try {
    ...
    box.innerHTML = bills.slice(0, 5).map(b => `
      <div class="history-row" data-id="${b.id}">
        ...
        ${(b.paid_by_identity_id || b.creator_identity_id) === state.identity.id ? `<button class="btn-sm delete-bill" data-id="${b.id}" data-title="${esc(b.title)}" title="Hapus bill" style="background:var(--red-bg);color:var(--red);flex-shrink:0;">🗑️</button>` : ""}
      </div>`).join("");
