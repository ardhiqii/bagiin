...
function renderEditItems() {
  ...
  elList.innerHTML = editState.items.map((it, idx) => `
    <div class="item-row" style="border-bottom:1px solid var(--border);flex-wrap:wrap;">
      ...
      <div style="flex-basis:100%;padding:6px 0 0;display:flex;align-items:center;gap:8px;">
        ...
        ${it.discount > 0 ? `<span class="muted" style="font-size:12px;">→ bayar ${rupiahFmt(Math.max(0, it.price - it.discount))}</span>` : ""}
      </div>
  ...
  $$("[data-role=discount]", elList).forEach(inp => bindRupiahInput(inp, (v) => {
    editState.items[+inp.dataset.idx].discount = v;
    updateEditTotal();
  }));
...
function updateEditTotal() {
  ...
  if (warn) {
    if (sumItems !== subtotal) {
      ...
      if (sumItems === subtotal + tax + service) {
        warn.textContent = `Harga item (${fmt(sumItems)}) kayaknya udah TERMASUK pajak, tapi lu isi Subtotal ${fmt(subtotal)} + PPN. Biar gak dobel: set Subtotal = ${fmt(sumItems)} dan PPN = 0.`;
      } else {
