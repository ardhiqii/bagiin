...
// ---------- Verify OCR result ----------
let verifyState = { items: [], subtotal: 0, tax: 0, service: 0, total: 0, photo_path: null, title: "", merchant: "", transacted_at: "", manual: false, paid_by_name: null };
...
function renderVerifyItems() {
  ...
  $$("[data-role=discount]", elList).forEach(inp => bindRupiahInput(inp, (v) => {
    verifyState.items[+inp.dataset.idx].discount = v;
    updateVerifyTotal();
  }));
...
function updateVerifyTotal() {
  ...
  if (warn) {
    if (!verifyState.manual && sumItems !== subtotal) {
      ...
      if (sumItems === rupiahParse($("#total-display").textContent.replace(/\D/g, ""))) {
