...
function itemRowHtml(it, data, mySel, myName) {
  ...
  if (isSlot) {
    ...
    const perServing = Math.floor(it.price_idr / totalQty);
    const mineTxt = myQty > 0 ? ` · kamu ${myQty}×` : "";
...
function perServingEst(it, othersQty, myQty) {
  ...
  return totalQty > 0 ? Math.floor(it.price_idr / totalQty) : it.price_idr;
}
function computeMyBreakdown(data, selQty) {
  ...
  data.items.forEach(it => {
    ...
    const totalQty = othersQty + myQty;
    if (myQty > 0 && totalQty > 0) sub += Math.floor(it.price_idr / totalQty) * myQty;
