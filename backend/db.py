...
def init_db():
    ...
    # migration: shared payment-method cashback (v84). Keep this separate from
    # the printed/order-level voucher so old bills continue to reconcile with
    # cashback=0.
    bcols = {r[1] for r in conn.execute("PRAGMA table_info(bill)").fetchall()}
    if "cashback_idr" not in bcols:
        conn.execute(
            "ALTER TABLE bill ADD COLUMN cashback_idr INTEGER NOT NULL DEFAULT 0")
    ...
    try:
        ...
        conn.execute(
            """INSERT INTO bill (id, creator_identity_id, title, merchant, transacted_at,
               ...
               order_discount_idr, cashback_idr, total_idr, tax_mode, participant_count,
               tax_included)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (bill_id, creator_id, title, merchant, transacted_at, photo_path,
             paid_by_name, subtotal, tax, service, order_discount, cashback, total,
...
def update_bill(bill_id: str, title: str, merchant=UNCHANGED,
    ...
    set_cols = ["title = ?", "subtotal_idr = ?", "tax_idr = ?", "service_idr = ?",
                "order_discount_idr = ?", "cashback_idr = ?", "total_idr = ?",
                "tax_included = ?"]
